...
function CatsItem({ cat, i,  likeCat, Filter,  }) {
  ...
  return (
    <>
      <div className={
        Filter == "All" ? ( isLiked ? styles.cat__itemActive : styles.cat__item ) : styles.cat__item }>
        <img src={ cat.url } className={ styles.cat__img } alt="Изображение кошки"/>
        <IconHeartHover className={ styles.cat__iconHeartHover }/>
        <IconHeartClicked className={
          Filter == "All" ? ( isLiked ? styles.cat__iconHeartClickedActive :
          styles.cat__iconHeartClicked) : styles.cat__iconHeartClicked }
                          onClick={ liked }/>
      </div>
    </>
  );
